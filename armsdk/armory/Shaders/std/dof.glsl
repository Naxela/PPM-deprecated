...
vec3 color(vec2 coords, const float blur, const sampler2D tex, const vec2 texStep, float fringe, float threshold, float gain) {
	vec3 col = vec3(0.0);
...
vec3 dof(const vec2 texCoord, const float gdepth, const sampler2D tex, const sampler2D gbufferD, const vec2 texStep, const vec2 cameraProj, float dofDistance, float dofFstop, float dofLength, float dofFocusX, float dofFocusY, float dofCoc, float dofMaxblur, float dofThreshold, float dofHighlight, float dofBias, float dofFringe, float dofDither) {
	float depth = linearize(gdepth, cameraProj);
	float fDepth = dofDistance;
	//float fDepth = linearize(texture(gbufferD, focus).r * 2.0 - 1.0, cameraProj); // Autofocus
	
	const float f = dofLength; // Focal length in mm
	const float d = fDepth * 1000.0; // Focal plane in mm
	float o = depth * 1000.0; // Depth in mm
	float a = (o * f) / (o - f); 
	float b = (d * f) / (d - f); 
	float c = (d - f) / (d * dofFstop * dofCoc); 
	float blur = abs(a - b) * c;
	blur = clamp(blur, 0.0, 1.0);
	
	vec2 noise = rand2(texCoord) * dofDither * blur;
	float w = (texStep.x) * blur * dofMaxblur + noise.x;
	float h = (texStep.y) * blur * dofMaxblur + noise.y;
	vec3 col = vec3(0.0);
	if (blur < 0.05) {
		...
		for (int i = 1; i <= rings; ++i) {   
			...
			for (int j = 0 ; j < ringsamples; ++j) {
				...
				col += color(texCoord + vec2(pw * w, ph * h), blur, tex, texStep, dofFringe, dofThreshold, dofHighlight) * mix(1.0, (float(i)) / (float(rings)), dofBias) * p;  
				s += 1.0 * mix(1.0, (float(i)) / (float(rings)), dofBias) * p;  
			}
		}
		col /= s;
	}
