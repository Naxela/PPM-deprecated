...
vec4 traceCone(sampler3D voxels, vec3 origin, vec3 dir, const float aperture, const float maxDist, const vec3 normal) {
	...
	while (sampleCol.a < 1.0 && dist < maxDist) {
		...
		dist += max(diam / 2, ((2.0 / voxelgiResolution.x) * voxelgiStep)); // Step size
		diam = dist * aperture;
	}
	return sampleCol;
}

vec4 traceDiffuse(const vec3 origin, const vec3 normal, sampler3D voxels, float vxgiOcc) {
	...
	#ifdef _VoxelCones1
	return traceCone(voxels, origin, normal, aperture, MAX_DISTANCE, normal) * vxgiOcc;
	#endif
...
float traceConeAO(sampler3D voxels, const vec3 origin, vec3 dir, const float aperture, const float maxDist) {
	dir = normalize(dir);
	float sampleCol = 0.0;
	float dist = 1.5 * ((2.0 / voxelgiResolution.x) * voxelgiStep) * voxelgiOffset;
	float diam = dist * aperture;
	vec3 samplePos;
	while (sampleCol < 1.0 && dist < maxDist) {
		...
		dist += max(diam / 2, ((2.0 / voxelgiResolution.x) * voxelgiStep));
		diam = dist * aperture;
	}
	return sampleCol;
}

float traceAO(const vec3 origin, const vec3 normal, sampler3D voxels, float vxgiOcc) {
	const float angleMix = 0.5f;
	const float aperture = 0.55785173935;
	vec3 o1 = normalize(tangent(normal));
	vec3 o2 = normalize(cross(o1, normal));
	...
	#ifdef _VoxelCones1
	return traceConeAO(voxels, origin, normal, aperture, MAX_DISTANCE) * vxgiOcc;
	#endif
