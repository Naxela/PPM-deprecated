...
uniform float rtgiStep;
uniform float rtgiStrength;
uniform int rtgiMaxSteps;
...
#ifdef _SSGICone9
float strength = 2.0 * (1.0 / (rtgiStrength * 10));
#else
float strength = 2.0 * (1.0 / (rtgiStrength * 10)) * 1.8;
#endif
...
void rayCast(vec3 dir) {
	...
	float dist = 1.0 / 5.0;
	#endif
	for (int i = 0; i < rtgiMaxSteps * 10; i++) {
		hitCoord += dir;
	...
	occ += dist;
	#ifdef _RTGI
	col += texture(gbuffer1, coord).rgb * ((rtgiStep * 10 * rtgiMaxSteps) - dist);
	#endif
}
