...
uniform float vxgiDiff;
uniform float vxgiSpec;
uniform float vxgiOcc;
uniform float vxgiEnv;
uniform float vxgiStep;
uniform float vxgiRange;
uniform float vxgiOffset;
...
void main() {
	...
	#ifdef _VoxelGITemporal
	vec4 indirectDiffuse = traceDiffuse(voxpos, n, voxels, vxgiOcc, vxgiStep, vxgiRange, vxgiOffset) * voxelBlend + traceDiffuse(voxpos, n, voxelsLast, vxgiOcc, vxgiStep, vxgiRange, vxgiOffset) * (1.0 - voxelBlend);
	#else
	vec4 indirectDiffuse = traceDiffuse(voxpos, n, voxels, vxgiOcc, vxgiStep, vxgiRange, vxgiOffset);
	#endif

	//fragColor.rgb = indirectDiffuse.rgb * voxelgiDiff * g1.rgb;

	fragColor.rgb = indirectDiffuse.rgb * vxgiDiff * g1.rgb;

	if (occspec.y > 0.0) {
		vec3 indirectSpecular = traceSpecular(voxels, voxpos, n, v, metrough.y, vxgiStep, vxgiRange, vxgiOffset);
		indirectSpecular *= f0 * envBRDF.x + envBRDF.y;
		fragColor.rgb += indirectSpecular * vxgiSpec * occspec.y;
	}
	...
	#ifdef _VoxelGITemporal
	envl.rgb *= 1.0 - (traceAO(voxpos, n, voxels, vxgiOcc, vxgiStep, vxgiRange, vxgiOffset) * voxelBlend + traceAO(voxpos, n, voxelsLast, vxgiOcc, vxgiStep, vxgiRange, vxgiOffset) * (1.0 - voxelBlend));
	#else
	envl.rgb *= 1.0 - traceAO(voxpos, n, voxels, vxgiOcc, vxgiStep, vxgiRange, vxgiOffset);
	#endif
	
#endif

#ifdef _VoxelGI
	fragColor.rgb += envl * vxgiEnv;
#else
	fragColor.rgb = envl;
#endif
