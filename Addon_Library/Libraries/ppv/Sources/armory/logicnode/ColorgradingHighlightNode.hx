...
class ColorgradingHighlightNode extends LogicNode {
	...
	override function run(from:Int) {
        ...
        if(property0 == "Uniform"){
            ...
            for (i in 0...5){
                arm.renderpath.PPV.colorgrading_highlight_uniforms[i][0] = inputs[i+2].get();
                arm.renderpath.PPV.colorgrading_highlight_uniforms[i][1] = inputs[i+2].get();
                arm.renderpath.PPV.colorgrading_highlight_uniforms[i][2] = inputs[i+2].get();
            }
            ...
            for (i in 0...5){
                arm.renderpath.PPV.colorgrading_highlight_uniforms[i][0] = inputs[i+2].get().x;
                arm.renderpath.PPV.colorgrading_highlight_uniforms[i][1] = inputs[i+2].get().y;
                arm.renderpath.PPV.colorgrading_highlight_uniforms[i][2] = inputs[i+2].get().z;
            }
